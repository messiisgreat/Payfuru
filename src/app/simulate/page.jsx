...
const Simulate = () => {
  ...
  useEffect(() => {
    if(locals.length === 0) {
      setSublocals(<p className="text-center text-red-700">利用可能な自治体はありません</p>)}
      else {
        setSublocals(locals.map((sublocal) => (
          <Link className="p-4 bg-grayBorder hover:bg-gray-400 m-5 rounded" href={`/local/${sublocal.id}`}>{sublocal.name}</Link>
        )))
      }
  }, [locals])
  ...
  return (
    <main className="bg-[url('../../public/background-1.jpg')]">
      <h1 className="text-[42px]  text-center py-16 text-white">寄附額シミュレーター</h1>
